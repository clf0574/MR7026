...
//===============面板开关输入======================
#define RCC_GPIO_ALL   RCC_APB2Periph_GPIOA
...
{
	if((GPIO_ReadInputDataBit(GPIO_PORT_KEY1,GPIO_PIN_KEY1)) == 0)
	{
		return 1;
	}
	else
...
static void bsp_InitKeyHard(void)
{
	...
	// 第1步：打开GPIO时钟 
	RCC_APB2PeriphClockCmd(RCC_GPIO_ALL, ENABLE);
...
static void bsp_InitKeyVar(void)
{
	...
	//s_tBtn[0].LongTime = 200;
  s_tBtn[0].RepeatSpeed = 0;	/* 每隔50ms自动发送键值 */
