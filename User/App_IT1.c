...
//=========================================================================================================
//¶ÅÑÇÓÐÏßÖÇÄÜ¼Ò¾Ó-ITÏµÍ³-µÚÒ»´úÐ­Òé

//µ±×ª·¢Æ÷ÓëËøÁ¬½Ó£¬ÎªÒ»¶ÔÒ»¹ØÏµ£¬¹²ÓÐ16¸öÍ¨µÀ£¬Ã¿¸öÍ¨µÀ±£´æµÄÊÇ¿ªÆôµÄ·½Ê½»òÃÜÂë
...
unsigned char DEV_INFO[DEV_INFO_NUM]={'M','R','3','2','0','0','-','1','8','-','0','3','-','2','7','-','S','1','.','0','2',};
unsigned char DEV_INFO_1[DEV_INFO_NUM]={'M','R','7','0','2','6','-','1','8','-','0','3','-','1','2','-','S','1','.','0','2',};
unsigned char DEV_INFO_2[DEV_INFO_NUM]={'M','R','3','x','0','0','-','1','8','-','0','1','-','1','0','-','S','1','.','0','2',};
...
void IT1_Load_RS485_InfoReport(void);//²úÆ·ÐÅÏ¢ÉÏ±¨
...
//		{
//			//================================================================================
//			index=(ucKeyCode-1)/3;							//°´¼üÍ¨µÀ0~7					
//			key_value=(ucKeyCode-1)%3;					//°´¼üÖµ0-Ñ¹ÏÂ£¬1-µ¯Æð£¬2-³¤°´	
//			//·ÇÉèÖÃ×´Ì¬
...
void IT1_ParseFrame(void)
{
  ...
  {
			RxDat.cnt=0;						                        		//Çå½ÓÊÕÊý¾Ý¸öÊý
			return;								                        			//Ìø³ö
	}
		
	CheckSum=Crc8CheckSum(RxDat.buf,(RxDat.cnt-1));		    	//¼ÆËãCrc8Öµ
	if(CheckSum==RxDat.buf[RxDat.cnt-1])				          	//ÅÐ¶ÏCrcÐ£ÑéÊÇ·ñÕýÈ·
	{
	  ...
	  if(Reg.fullflag==0)Led_set(0,12,12,1,1);
		...
		{
			//============================================================================
			case MSG_broadcast:					                        //¹ã²¥µØÖ·²»ÐèÒªÓ¦´ð				
				    IT1_Function_BroadCast();		
			break;
			//==============================================================================
      case MSG_command:					        	            		//ÃüÁî±¨ÎÄ
            IT1_Function_CMD();			
			break;
			//===============================================================================
			case MSG_request:										    						//ÇëÇó±¨ÎÄ
			...
			break;
			//=================================================================================
			case MSG_data:					    				        				//Êý¾Ý±¨ÎÄ 
			     	IT1_Function_Date();					            		//Òª×ª·¢µÄÊý¾Ý				
			break;
			//==================================================================================
			case MSG_setup:											    						//ÉèÖÃ±¨ÎÄ	
						IT1_Function_Setup(); 
			break;
			//==================================================================================
			case MSG_modify:							                			//RS485µØÖ·ÉèÖÃ		
						IT1_Function_Modfiy();
			break;
			//==================================================================================
			//20161226ÐÞ¸ÄÎªÖ§³ÖºìÍâÊý¾ÝÏÂÔØ
			case 0x06:
			     IT1_IR_Setup();
			break;	
			//===================================================================================
			...
			case 0x20:
						...
						else if(RxDat.buf[6]>0x01)
						{
							  ...
							  //=====================================
							  //2017.05.12Ôö¼ÓÏòÒ£¿ØÆ÷ÇëÇóºìÍâÅäÖÃÖ¸Áî
							  buf[0]=0x03;
							  buf[1]=Dev.rand;
							  buf[2]=0x01;
							  buf[3]=0x00;
...
// 									}
	...
	{	
				...
				//==================================
				//³¬Ê±´¦Àí
				if(Bus.timeout>=100)				//³¬¹ý100mS£¬Ã»ÊÕµ½×Ô¼º·¢³öµÄÊý¾Ý
				{
								    if(t_resend>0)
								{
											t_resend--;
	 ...
	 else if(send_step==3)					//µÈ´ýÊÕµ½Ó¦´ðÊý¾Ý
	 {
				...
				//===================================================================
				//Ó¦´ð³¬Ê±
				if(Bus.timeout>=150)				//³¬¹ý100mS£¬Ã»ÊÕµ½×Ô¼º·¢³öµÄÊý¾Ý
				{
								if(t_resend>0)
								{
										    t_resend--;
...
void IT1_Dev_Register(void)
{
	...
	if(Dev.stu<DEV_NORMAL)
	{
			...
			//ÉÏµç·¢ËÍÇëÇóÃüÁî
			ps->buf[0] =0x02;									//Ö¡¿ØÖÆ,´ÓÉè±¸ÇëÇóÃüÁî
			ps->buf[1] =0x00;
			ps->buf[2] =0x00;
			ps->buf[3] =Dev.serial++;					//Ö¡ÐòÁÐºÅ
			ps->buf[4] =Dev.addr[0];					//ÍøÂçID
			ps->buf[5] =0x21;									//ÍøÂç¿ØÖÆÃüÁî
			
			ps->buf[6] =(uint8_t)(Dev.type>>8);						//Éè±¸ÀàÐÍ
			ps->buf[7] =(uint8_t)(Dev.type&0x00ff);

			if((Dev.type&0xf000)==0xb000)ps->buf[8] =0x80;									//Éè±¸×ÜÊýÒÔ¼°Éè±¸×ÓµØÖ·
			else                         ps->buf[8] =0x10;
			
			ps->buf[9] =0x00;									//Êý¾Ý¸öÊý
			ps->buf[10]=0;										//Ð£ÑéºÍ
			

			ps->buf[10]=Crc8CheckSum((uint8_t*)ps->buf,10);	//CRC8Ð£Ñé
			...
			#else
			//ÉÏµç·¢ËÍÇëÇóÃüÁî
			//Êý¾Ý²¿·Ö 0x03 0x02 0x01 0x08 0x77 0x10 0x02 0x01 0x02 0x00 0x04 0x01 0x8b 0x00
			...
			ps->buf[8] =0x80;															//Éè±¸×ÜÊýÒÔ¼°Éè±¸×ÓµØÖ·
			ps->buf[9] =0;																//Êý¾Ý¸öÊý
			...
			{
						ps->buf[10]=NewRegType.num;							//Í¨µÀÊý
						ps->buf[11]=0x01;												//ÀàÐÍ
						index=0;len=0;
						for(i=0;i<8;i++)
						{
			...
			ps->buf[len+10]=Crc8CheckSum((uint8_t*)ps->buf,len+10);	//CRC8Ð£Ñé
...
//=========================================================================================================
//ÎÞÏßÍø¹Ø±»ÅäÖÃºó£¬Èç¹ûÎÞÏßÉè±¸Ã»ÓÐÉÏ±¨£¬ÔòÄ£·ÂÉÏ±¨Ò»´Î£¬Ê¹ÎÞÏßÉè±¸ÔÚÏß
void IT1_Dev_Declare(uint8_t index,uint8_t cmd)
{
 	    ...
 	    uint8_t buf[2],rt;
 			...
 			if(ps==NULL){return;}			
 			//ÉÏµç·¢ËÍÇëÇóÃüÁî
 			ps->buf[0] =0x05;									//Ö¡¿ØÖÆ,´ÓÉè±¸ÇëÇóÃüÁî
 			ps->buf[1] =(uint8_t)(Master_Address>>8);
 			ps->buf[2] =(uint8_t)(Master_Address&0xff);
 			ps->buf[3] =0x00;					        //
 			ps->buf[4] =Dev.addr[0];					//±¾µØµØÖ·
 			ps->buf[5] =index+1;					//Í¨µÀºÅ
      ...
      //if(rt==RE_ERR)return;
			...
			if((Exchangebuf.key!=0x00)&&(Exchangebuf.key!=0xff))
			{
					...
					ps->buf[11]=Crc8CheckSum((uint8_t*)ps->buf,11);	//CRC8Ð£Ñé
					ps->cnt=12;
			}
			else
		  {
					ps->buf[6] =0x03;						      //ÃüÁî×Ö½Ú
					ps->buf[7] =cmd;						
					ps->buf[8] =buf[0];								
					ps->buf[9] =buf[1];						
					ps->buf[10]=Crc8CheckSum((uint8_t*)ps->buf,10);	//CRC8Ð£Ñé
...
//==========================================================================================================
//¹ã²¥Ö¡´¦Àí
void IT1_Function_BroadCast(void)                                                   //¹ã²¥ÎÞÓ¦´ð
{
	      //±ØÐëÓÃ¾²Ì¬±äÁ¿
        _Frame *ps;
	      //uint8_t i;
        ...
        //===================================================================================================
        if(RxDat.buf[5]==0x01)                                                             //¹ã²¥²éÑ¯×´Ì¬
        {
                //Éè±¸ÎïÀíµØÖ·
								if((Dev.type&0xf000)==0xb000)
								{
										ps->buf[0] =0x04;                                                         //Ö¡¿ØÖÆ,´ÓÉè±¸ÇëÇóÃüÁî
										ps->buf[1] =(Master_Address>>8)&0x00ff;                       						//Ä¿±êµØÖ·
										ps->buf[2] =Master_Address;                                       				//Ä¿±êµØÖ·
										ps->buf[3] =0x00;                                                         //Ö¡ÐòÁÐºÅ
										ps->buf[4] =Dev.addr[0];                                             			//ÍøÂçID
										ps->buf[5] =0x10;                     
										ps->buf[6] =0x03;                                                         //Êý¾Ý³¤¶È
												
										ps->buf[7]=0x00;
										ps->buf[8]=0x00;																													//ÉèÖÃÎÂ¶È
										ps->buf[9]=0x00;																													//Êµ¼ÊÎÂ¶È	
											
										ps->buf[10]=Crc8CheckSum(ps->buf,10);                         						//CRC8Ð£Ñé
										ps->cnt=11;                                                               //·¢ËÍ×ÜµÄ×Ö½ÚÊý
										ps->flag=1;
										ps->resend=0;
										ps->ackwait=0;
										EnQueueBuf(&RS485_Queue,8);	
								}
								//Èç¹ûÊÇËø£¬ÔòÉÏ±¨µ±Ç°µÄ×´Ì¬ºÍËæ»úÃÜÂë
        ...
        //=====================================================================================================
        else if(RxDat.buf[5]==0x02)                                                        //¹ã²¥²éÑ¯ÔÚÏß
        {
                Dev.stu=DEV_REGISTER;
                RegTimeClr();					
                InitQueueBUF(&RS485_Queue);					
...
//=========================================================================================================
//ÃüÁîÖ¡´¦Àí
void IT1_Function_CMD(void)                                                           //Ö÷»ú·¢ËÍÃüÁî
{
        // _Frame *ps;
				uint8_t chl;
	      //ps=GetQueue_InQ(&RS485_Queue,8);
        ...
        switch (RxDat.buf[5])
        {
                ...
                case Dev_OnLine_CMD:                                            //²éÑ¯Éè±¸ÊÇ·ñÔÚÏß
                      //Êµ¼ÊµØÖ· 
                        if(RxDat.buf[7]>=4)
												{
													      IT1_Load_RS485_Ack(OPT_ERR);
                ...
                break;
                case ACK_CMD:                                                   //Ó¦´ðÃüÁî
                
                break;
								//===========================================================================================
								//20171104£¬ÕâÌõÖ¸ÁîÒÑ¾­Ã»ÓÃ
								//²é²úÆ·ÐòÁÐºÅ
								//Ö÷»ú:0x01 + 0x00+Addr + 0xXX + 0x00 + 0x09 + 0x01+0xXX+ CRC
								//´Ó»ú:0x06 + 0x0000 + 0xXX + Addr + 0xXX£¨Éè±¸×ÜÊý£©+ Êý¾Ý³¤¶È+ Êý¾Ý + CRC
								case 0x09:
...
//============================================================================================================
//Êý¾ÝÖ¡´¦Àí
//ËµÃ÷£º¶ÔÒ£¿ØÆ÷À´Ëµ£¬Êý¾ÝÖ¡ÓÃÓÚÉèÖÃÃüÁî£»¶Ô¿ØÖÆÉè±¸À´Ëµ£¬Êý¾ÝÖ¡ÓÃÓÚ¿ØÖÆ
//ÅäÖÃËæÉíÌùÊôÐÔ£º clf -Ö¡¿ØÖÆ=0x04
//Ö¡¿ØÖÆ(1)+Ä¿±êµØÖ·£¨2£©+Ö¡ÐòÁÐºÅ£¨1£©+ÍøÂçID£¨1£©+Éè±¸×ÜÊý£¨1£©+Êý¾Ý³¤¶È£¨1£©+ÊôÐÔÂë£¨1£©+CRC
//ÊôÐÔÂë£º0x01±íÊ¾±»ÅäÖÃ³ÉµÆ¹âËæÉíÌù,0x02±íÊ¾±»ÅäÖÃ³Éµç»úËæÉíÌù,0x03±»ÅäÖÃ³É³¡¾°ËæÉíÌù

void IT1_Function_Date(void)                                                                //Ö÷»ú·¢ËÍÊý¾ÝÖ¡ÃüÁî
{
	  ...
	  uint8_t i,cmd,userkey[8],len,flagok,tmp;
		...
		//================================================================================================
		Date_Num=RxDat.buf[6];                                                         			//Êý¾Ý×Ö½Ú¸öÊý
...
//				}
//				//==========================================================================================
//				else if(type==EX_DEV_SOCKET)   //ÖÇÄÜ²å×ù
//				{
//						if(cmd==0xe6)     //¹ØµÆ
//						{
//							  Wireless_Dev_Action(index,0x08,0,0);
...
//				}
//        //=========================================================================================				
//				else if(type==EX_DEV_LIGHT)    //µÆ¹â			
//				{
//					  if(cmd==0x01)     //¹ØµÆ
//						{
//							   Wireless_Dev_Action(index,0x08,0,0);
...
//				//==========================================================================================
//				//µ÷¹âµÆ add 2016.2.23
//				else if(type==EX_DEV_LIGHT_ADJ)    //µÆ¹â			
//				{
//					  if(cmd==0x01)     //¹ØµÆ
//						{
//							   Wireless_Dev_Action(index,0x08,0,0);      //ÕÔ¹¤µ÷¹âµÆ²»Ö§³Ö
...
//						else if(cmd==0x03)
//						{
//							  //Wireless_Dev_Action(index,0x1d,cmd_dat,0);  //ÕÔ¹¤ÕâÀïÓÃ0x19
//							  Wireless_Dev_Action(index,0x19,cmd_dat,0);  //ÕÔ¹¤ÕâÀïÓÃ0x19
				...
				{	
							if(Date_Num==1)
							{
										...
										if(cmd==0x05)
										{
													LockItem=lock.item;
													LockItem.safe =0x01;
											    //LockItem.stu|=0x02;			//²¼·ÀÊ±Í¬Ê±¿ªÆôÍÆËÍ£¬Í¬Ê±·¢¹ØÃÅÖ¸Áî
											    LockItem.stu|=0x06;
													Write_Lock_Table(0);
	
													IT1_Load_RS485_Ack(OPT_OK);

											    LockItem.stu&=0xfe;				//¹ØÃÅ
										...
										else if(cmd==0x06)
										{
											    ...
											    LockItem.stu&=0xf9;				//20180328 ³··ÀÍ¬Ê±È¥µôÍÆËÍ
							...
							if((cmd==0x01)||(cmd==0x02))		//¿ªËø ºÍ¹ØËø
							{
										//Î¹¹·
										IWDG_Feed();	
										//Ó¦´ð
...
//							}
							...
							tmp=lock.item.stu;
//Êµ¼ÊÔ­ÒòÎªAPP·¢¹ØÖ¸ÁîÊ±£¬Ä£Äâ·¢ÁËÒ»¸ö¹ØÖ¸Áî£¬ÒÔ±ãAPPÄÜ»Ø¸´²Ù×÷£¬Ôì³ÉÃÅËøÊµ¼ÊºÍ×ª·¢Æ÷²»Ò»ÖÂ
...
void IT1_Function_Setup(void)
{
    ...
    if(Dev.stu==DEV_SETUP)
    {
		...
		{
				len=RxDat.buf[6];	
				type=RxDat.buf[7];						       //ÃüÁî	
				keynum=RxDat.buf[10];								 //Í¨µÀ»ò°´¼ü¸öÊý
				index=RxDat.buf[5]&0x0f;
				if((index==0)||(index>8))                                
				{
							IT1_Load_RS485_Ack(OPT_ERR); 
				...
				switch(type)
				{
					...
					tmp=IT1_RS4852Wireless(RxDat.buf[8],RxDat.buf[9]);												//Ö÷»úÒªÇóÅä¶ÔµÄÉè±¸ÀàÐÍ
					...
					//===============================================================================	
					//ÒªÇóÔö¼ÓµÄÐéÄâÉè±¸ÊÇÒ£¿ØÆ÷
					if(Iscode==1)
					{
									//²»ÒªÇóÎÞÏß·¢ËÍ£¬Ö»µÈ´ýÒ£¿ØÆ÷·¢ËÍ
									Exchangebuf.type= tmp;                                    //Éè±¸ÀàÐÍ
									Exchangebuf.addr= index-1;;
									Exchangebuf.channel=RxDat.buf[11];
									Exchangebuf.wl_addr[0]=	0;				                  			//µÈ´ý½ÓÊÕÒ£¿ØÆ÷µÄµØÖ·
									Exchangebuf.wl_addr[1]= 0;  		                      		//
									Exchangebuf.wl_addr[2]= 0;  
						
									if(len==4)Exchangebuf.key=keynum;													//°´¼ü¸öÊý
									else      Exchangebuf.key=4;	
									Led_set(1,0xff,1,0xff,2);																	//³¤ÁÁ
					}
		//			//ÒªÇóÔö¼ÓµÄÐéÄâÉè±¸ÊÇ¿ØÖÆÉè±¸
		//			else if(Iscode==2)
		//			{          
		//							Exchangebuf.type= tmp;                                    //Éè±¸ÀàÐÍ
		//							Exchangebuf.addr= index-1;;            
		...
		//							Exchangebuf.wl_addr[1]= (Dev.rand);  	                  	//Ëæ»úÊý-Ò£¿ØÆ÷µØÖ·1 
		//							//Exchangebuf.wl_addr[2]= (~Dev.rand|RxDat.buf[8]);       //Ëæ»úÊý-Ò£¿ØÆ÷µØÖ·2    
		//							Exchangebuf.wl_addr[2]=Dev.serial++;			
		//							Wireless_Dev_Action(Exchangebuf.addr,0x01,0,0);         	//×°ÎÞÏß·¢ËÍÊý¾Ý
		//			}
		//			//Éè±¸Î´Ê¶±ð
					else
					{
					...
					case 0xcc:
						...
						Exchangebuf.wl_addr[1]=Ex_Table.Exchange[index].wl_addr[1];
						Exchangebuf.wl_addr[2]=Ex_Table.Exchange[index].wl_addr[2];
						//Wireless_Dev_Action(index,0x02,0,0);         //×°ÎÞÏß·¢ËÍÊý¾Ý
							    ...
							    if(type==0xbb)
									...
									else
									{
										    ...
										    IT1_Lock_DataLoad(lock.index,0x02,0xcc);
												LockItem.Serail[lock.index-1]=0x00;    //ÓÃ»§±àºÅ+IDÀàÐÍ+ÓÃ»§ÀàÐÍ;
...
uint8_t IT1_Modify_Type(void)
{
	uint8_t t1,t2;
	uint16_t tt;
	//ÒòÎªÐÞ¸ÄÖ¸Áî0x08ÔÚ¹ã²¥ÉèµØÖ·Ê±£¬²»ÆÁ±Î
	...
	if((tt==0xb100)||(tt==0xb140)||((tt&0xf000)==0x3000))		//Ôö¼ÓËø×ª·¢Æ÷Ö§³Ö
	{
...
void IT1_Load_RS485_Ack(unsigned char st)
{
	...
	TxDat.buf[0]=0x03;														//Ö¡¿ØÖÆ-Ó¦´ð±¨ÎÄ
	TxDat.buf[1]=(Master_Address>>8)&0x00ff;			//Ä¿±êµØÖ·H
	TxDat.buf[2]=Master_Address;									//Ä¿±êµØÖ·L

  TxDat.buf[3]=0X00;														//Ö¡ÐòÁÐºÅ

	TxDat.buf[4]=RxDat.buf[2];										//ÍøÂçID
	
			if((st==OPT_OK)||(st==RE_OK))							//´íÎóÓ¦´ð
			{
					TxDat.buf[5]=0x20;		
					TxDat.buf[6]=0x00;														//Êý¾Ý¸öÊý
					TxDat.buf[7]=Crc8CheckSum(TxDat.buf,7);				//CRC8Ð£Ñé
					TxDat.cnt=8;																	//·¢ËÍ×ÜµÄ×Ö½ÚÊý
					TxDat.f_tx=1;		
			}
			else								//Õý³£Ó¦´ð
			{
				TxDat.buf[5]=0x20;						//ÍøÂç¿ØÖÆÃüÁî
				TxDat.buf[6]=0x01;	
				if(st==OPT_ERR_SETUP)TxDat.buf[7]=0xf3;	
				else                 TxDat.buf[7]=0xf1;	
				TxDat.buf[8]=Crc8CheckSum(TxDat.buf,8);				//CRC8Ð£Ñé
				TxDat.cnt=9;																	//·¢ËÍ×ÜµÄ×Ö½ÚÊý
				TxDat.f_tx=1;					
			}
}
...
void IT1_Load_RS485_InfoReport(void)
{
	  ...
	  uint8_t *p;
		...
		ps->buf[0]=0x06;			//Ö¡¿ØÖÆ
		ps->buf[1]=(Master_Address>>8)&0x00ff;  //Ä¿±êµØÖ·H
		ps->buf[2]=Master_Address;		//Ä¿±êµØÖ·L
		ps->buf[3]=0x00;	
		ps->buf[4]=Dev.addr[0];
		ps->buf[5]=0x88;		//Í¨µÀºÅ
		ps->buf[6]=0x08;						//Êý¾Ý³¤¶È
		
		//2016.5.12ÐÞ¸Ä
		//for(i=0;i<8;i++)ps->buf[7+i]=(uint8_t)(IT_INFO[i]);								//²úÆ·ÐÅÏ¢
		...
		ps->buf[15]=Crc8CheckSum(ps->buf,15);		//CRC8Ð£Ñé
		ps->cnt=16;					                 //·¢ËÍ×ÜµÄ×Ö½ÚÊý
		
		ps->flag=1;
		ps->resend=4;
		//ps->rand=100;
		ps->ackwait=30;					//µÈ´ýÓ¦´ðÊ±¼ä×î´ó300mS
		EnQueueBuf(&RS485_Queue,8);
}
...
//							}
				...
				ps->buf[9]=Crc8CheckSum(ps->buf,9);		//CRC8Ð£Ñé
				ps->cnt=10;					//·¢ËÍ×ÜµÄ×Ö½ÚÊý
...
//				}
				...
				if((Ex_Table.Exchange[index].type==EX_KEY_NORMAL1)||(Ex_Table.Exchange[index].type==EX_KEY_SCENE1))
				{
							ps->buf[6]=1;                        	  //Êý¾Ý³¤¶È
							ps->buf[7]=cmd+1;												//¼üÖµ
							ps->buf[8]=Crc8CheckSum(ps->buf,8);		//CRC8Ð£Ñé
							ps->cnt=9;					//·¢ËÍ×ÜµÄ×Ö½ÚÊý
				}
				//ÖÇÄÜ
...
//						ps->buf[3]=0x00;	
//						ps->buf[4]=Dev.addr[0];
//						ps->buf[5]=index;		//Í¨µÀºÅ	
//						ps->buf[6]=9;                        	  //Êý¾Ý³¤¶È
//						ps->buf[7]=dat;												  //¼üÖµ
//						for(i=0;i<8;i++)ps->buf[8+i]=lock.randkey[i];												//Ä£Ê½£¬ÕâÀïÄ¬ÈÏÎªµ¥»÷
//						ps->buf[16]=Crc8CheckSum(ps->buf,16);		//CRC8Ð£Ñé
//						ps->cnt=17;					//·¢ËÍ×ÜµÄ×Ö½ÚÊý
//						lock.report=1;
				...
				else if(type==0x04)
				{
						...
						ps->buf[3] =Dev.serial++;							//Ö¡ÐòÁÐºÅ
						ps->buf[4] =Dev.addr[0];							//ÍøÂçID
						ps->buf[5] =0x10+index;											//Éè±¸×ÜÍ¨µÀºÍµ±Ç°Í¨µÀ
						ps->buf[6] =01;						      			//Êý¾Ý³¤¶È
				    ps->buf[7]=dat;	
						ps->buf[8]=Crc8CheckSum(ps->buf,8);		//CRC8Ð£Ñé
						ps->cnt=9;					                 	//·¢ËÍ×ÜµÄ×Ö½ÚÊý  
...
////			
////		
////		    //2016.5.10Ôö¼ÓÍ¨ÓÃËæÊ±Ìù
////				if((Ex_Table.Exchange[index].type==EX_KEY_SCENE)||(Ex_Table.Exchange[index].type==EX_KEY_GENERAL))	//³¡¾°ËæÉíÌù+Í¨ÓÃËæÊ±Ìù
////				{
////							ps->buf[6]=1;                        	//Êý¾Ý³¤¶È
////							ps->buf[7]=dat;
////							ps->buf[8]=Crc8CheckSum(ps->buf,8);		//CRC8Ð£Ñé
////							ps->cnt=9;					//·¢ËÍ×ÜµÄ×Ö½ÚÊý
////				}                                       
////        else if(Ex_Table.Exchange[index].type==EX_KEY_LIGHT)  //µÆ¹âËæÉíÌù                              
////        {
////							ps->buf[6]=2;                         	//Êý¾Ý³¤¶È
////							ps->buf[7]=dat;
...
////    		      ps->cnt=10;					//·¢ËÍ×ÜµÄ×Ö½ÚÊý
////        }
////				else if(Ex_Table.Exchange[index].type==EX_KEY_MOTOR)								//µç»úËæÐÄÌù
////				{
////							ps->buf[6]=2;                         	//Êý¾Ý³¤¶È
////							ps->buf[7]=dat;
...
////							ps->cnt=10;					//·¢ËÍ×ÜµÄ×Ö½ÚÊý
////				}
////				//2016.9.7Ôö¼Ó·ç¹âÓê´«¸ÐÆ÷ ×ÛºÏ
////				else if(Ex_Table.Exchange[index].type==EX_DEV_WSR)								//µç»úËæÐÄÌù
////				{
////							ps->buf[6]=3;                         	//Êý¾Ý³¤¶È
////					    //È¡µÃ·ç¹âÓêÊý¾Ý
...
////									
////									ps->buf[9]=wlrbuf[1];
////									ps->buf[5]=0x30+wlrbuf[0];								//Í¨µÀºÅ Í¨µÀ0-·ç£¬Í¨µÀ1-¹â£¬Í¨µÀ2-Óê
////							
////									ps->buf[10]=Crc8CheckSum(ps->buf,10);		//CRC8Ð£Ñé
////									ps->cnt=11;					//·¢ËÍ×ÜµÄ×Ö½ÚÊý
////								
...
////	}
////	//×°ÈëÉè±¸Ö÷¶¯ÉÏ±¨Êý¾Ý
////	#if 0    //clf mod 20161216Ö»×ö´«¸ÐÆ÷
////	else 
////	{
////								dat=cmd;
////								//if(dat>100)dat=100;				//Ã»ÐÐ³ÌÊ±ÉÏ±¨Îª0xff
////							  ps->buf[6]=3;
////	
////								if(Ex_Table.Exchange[index].type==EX_DEV_LIGHT)	            //µÆ¹âÉÏ±¨
////                {
...
////                }
////								//==================================================================
////								//µ÷¹âµÆÉÏ±¨ add 2016.2.23
////								else if(Ex_Table.Exchange[index].type==EX_DEV_LIGHT_ADJ)	  //µ÷µÆ¹âÉÏ±¨
////                {
////									if(dat>100)dat=100;
////									
////                	if(dat==0x00)
...
////                }
////								
////                else if(Ex_Table.Exchange[index].type==EX_DEV_SOCKET)	//ÖÇÄÜ²å×ù
////                {
...
////         } 
////	//=========================================================================================================
////	#endif  //clf mod 20161216Ö»×ö´«¸ÐÆ÷×ª·¢Æ÷
...
void IT1_RS485_StuLoad(uint8_t index,uint8_t len,uint8_t *pdat)
{
	      ...
	      uint8_t i;
				...
				ps->buf[3]=0x00;	
				ps->buf[4]=Dev.addr[0];
				ps->buf[5]=0x10;//0x80+index+1;		//Í¨µÀºÅ
				...
				ps->buf[7+len]=Crc8CheckSum(ps->buf,7+len);		//CRC8Ð£Ñé
				ps->cnt=8+len;					//·¢ËÍ×ÜµÄ×Ö½ÚÊý
...
void IT1_AddressSetting(void)
{
   ...
   if(IsModify==1)
   {
        Dev.addr[0]=RxDat.buf[10];
				...
				//if((Dev.err&~ERR_POWER)==0)										//µçÑ¹Õý³£²ÅÄÜÐ´£¬µÍÑ¹½ûÖ¹
				{
				...
				if(RxDat.buf[10]!=RxDat.buf[2])
				{
						for(i=0;i<8;i++)
						{
									...
									Wireless_Dev_Action(i,0x02,0,0);         //×°ÎÞÏß·¢ËÍÊý¾Ý
...
void IT1_Modify_Addr_Old(void)
{
    ...
    { 		
					TxDat.buf[0] =0xCD;						//Ö¡¿ØÖÆ,´ÓÉè±¸ÇëÇóÃüÁî
					TxDat.buf[1] =0x00;						//Ä¿±êµØÖ·
					TxDat.buf[2] =0x0A;						//Ä¿±êµØÖ·
					TxDat.buf[3] =0x03;						//Ö¡ÐòÁÐºÅ
					TxDat.buf[4] =0X00;						//Éè±¸×ÜÊýÒÔ¼°Éè±¸×ÓµØÖ·
					TxDat.buf[5] =0X00;						//ÍøÂçID£¬
					TxDat.buf[6] =0X00;						//Êý¾Ý¸öÊý  	
					TxDat.buf[7] =RxDat.buf[7];				//Êý¾Ý¸öÊý 
					TxDat.buf[8] =RxDat.buf[8];				//Êý¾Ý¸öÊý 
                ...
                Write_Dev(Dev.addr[0],TYPE_EE_ADDR_ADDR0);							
					...
					InitQueueBUF(&RS485_Queue);					//Çå¶ÓÁÐ	
          InitQueueBUF(&WirelessSend_Queue);					//Çå¶ÓÁÐ						
					Dev.stu=DEV_REGISTER;
					RegTimeClr(); 
					TxDat.f_tx=1;
	   }
...
//============================================================================================================
//³ÌÐòÉý¼¶¸üÐÂ,ÏÈ¸´Î»£¬½øÈëBOOTLOADER³ÌÐò
void IT1_PROGRAM_Uadata(void)
{
...
//	if((pa1==0x72)&&(pa2==0x40))			//ÎÞÏßµÆ¹âÃæ°å
// 	{
// 		tmp=EX_KEY_LIGHT1;
// 	}
//	else if((pa1==0x71)&&(pa2==0x40))//ÎÞÏß³¡¾°Ãæ°å
// 	{
// 		tmp=EX_KEY_SCENE1;
// 	}
//	else if((pa1==0x72)&&(pa2==0x60))//ÎÞÏß³¡¾°Ãæ°å
// 	{
// 		tmp=EX_KEY_MOTOR1;
// 	}
	if((pa1==0x75)&&(pa2==0x40))//ÎÞÏßÍ¨ÓÃÃæ°åå
 	{
 		tmp=EX_KEY_NORMAL1;
 	}
	else if((pa1==0x77)&&(pa2==0x40))//ÎÞÏßÖÇÄÜÃæ°å
 	{
 		tmp=EX_KEY_FULL1;
 	}
...
// 		tmp=EX_DEV_LIGHT;
// 	}
//	else if((pa1==0x02)&&(pa2==0x00))				//µ÷¹âµÆ
// 	{
...
// 	}
  ...
  //{
 	//	tmp=EX_DEV_BLIND_R;
  //}
//	else if((pa1==0x13)&&(pa2==0x30))       //µç×ÓEÐÍÊÖ¶¯
// 	{
// 		tmp=EX_DEV_MOTOR_E;	
// 	}
...
//============================================================================================================
//2016.12.26ºìÍâÊý¾Ý´¦Àí
//
void IT1_IR_Setup(void)
{
						uint8_t tmp;
						//===========================================================================================================
						//2017.1.6ÐÞ¸Ä£¬ºìÍâ½âÂë²¿·Ö·ÅÔÚÒ£¿ØÆ÷ÉÏ
						__IO uint8_t i;
	          ...
	          Ir.keychm=0;
			      for(i=0;i<Ir.keylen;i++)
						...
						Ir.flag=1;																	//±íÊ¾Êý¾Ý×°ÈëÍê±Ï
						Ir.sendtime=SEND_TIME_MAX;									//×î´ó·¢ËÍÊ±¼ä
						IT1_Load_RS485_Ack(OPT_OK); 
...
//=============================================================================================================
//ºìÍâÅäÖÃÍê³Éºó£¬ÏòPC·¢ËÍÅäÖÃÇé¿ö,Èç¹ûÎÞÏß²¿·ÖÊ§°Ü£¬ÔòÊý¾ÝÈ«²¿Ìî³ä0xff
void IT1_IrSetup_Report(uint8_t st)
{
				...
				{
							ps->buf[6]=2+Ir.typelen+Ir.keylen+2;						//×ÜµÄÊý¾Ý³¤¶È,ÔÚºóÃæÖØÐÂ¼ÆËã
							ps->buf[7]=Ir.cmd_key;
...
void IT1_IrDel_Report(uint8_t index)
{
				...
				ps->buf[6]=0x03;						//×ÜµÄÊý¾Ý³¤¶È,ÔÚºóÃæÖØÐÂ¼ÆËã
				ps->buf[7]=0x01;						//É¾³ý	
				ps->buf[8]=Ir.cmd_key;
				ps->buf[9]=0x00;
...
void IT1_IR_LoadToWireless(uint8_t chl)
{
	      ...
	      if((Ir.wait==0)&&(Ir.flag==1))
	      {
					...
					pws->buf[0]=0x30;														//°ü³¤¶È
					pws->buf[1]=0x40;														//°üÀàÐÍ
					
					//2017.11.16ÐÞ¸Ä0Îªchl
					for(i=0;i<3;i++)pws->buf[2+i]=Ex_Table.Exchange[chl].wl_addr[i]; //Ä¿±êµØÖ·
					...
					pws->buf[12]=Ir.cmd_key;									//¼üÖµ»òÃüÁî
					
					//ÏÂ·¢µÄÊý¾ÝÊÇÒ£¿ØÆ÷ÀàÐÍ
					if(Ir.curtype==0)
					{
						len=Ir.typelen;											
						if((len-Ir.curps)>32)
					...
					else 
					{
								...
								//bit7 bit6 Çø·Öµ±Ç°Êý¾ÝÊÇÒ£¿ØÀàÐÍ»òÒ£¿ØÆ÷°´¼ü
...
void IT1_IR_UnLoadToWireless(uint8_t chl,uint8_t key,uint8_t mode)
{
	          ...
	          if(chl>=8)return;
					  ...
					  Ir.wait=1;																	//
						...
						pws->buf[0]=0x10;														//°ü³¤¶È
						pws->buf[1]=MTD_IR_DEL;														//°üÀàÐÍ
					
					  //2017.11.16Ôö¼ÓÍ¨µÀ£¬ÐÞ¸Ä0Îªchl
						for(i=0;i<3;i++)pws->buf[2+i]=Ex_Table.Exchange[chl].wl_addr[i]; //Ä¿±êµØÖ·
	          pws->buf[5]=Ex_Table.Exchange[chl].channel;
						for(i=0;i<4;i++)pws->buf[6+i]=Ir.TgAddr[i];											//54¼üÒ£¿ØÆ÷µØÖ·	Ô´µØÖ·
					  ...
					  pws->buf[11]=mode;									  //
					  pws->buf[12]=key;										//¼üÖµ»òÃüÁî
					
						pws->buf[13]=0x00;							//¸ß2 bit ÎªÀàÐÍ
...
//=====================================================================================================
//========================================================================================================
//RS485µã¶Ôµã²éÑ¯×´Ì¬£¬Ö±½ÓÓ¦´ð£¬×ß0x04±¨ÎÄ
void IT1_ExTable_Stu_Ack(uint8_t index)
{
			 uint8_t chl;
	    chl=index&0x0f;
	    ...
	    chl=chl-1;
			...
			TxDat.buf[0]=0x04;												//Ö¡¿ØÖÆ-Ó¦´ð±¨ÎÄ
			TxDat.buf[1]=0x00;//TxDat.buf[1]=(Master_Address>>8)&0x00ff;	//Ä¿±êµØÖ·H
			TxDat.buf[2]=Master_Address;							//Ä¿±êµØÖ·L

			TxDat.buf[3]=0X00;												//Ö¡ÐòÁÐºÅ

			TxDat.buf[4]=Dev.addr[0];								  //ÍøÂçID
	    ...
	    //ÊÇ´«¸ÐÆ÷ ÉÏ±¨ Ê¹ÄÜ×´Ì¬
			...
			{
					TxDat.buf[7]=0x01;														//ÊÇ×îºóÒ»¸öÃüÁî
				  TxDat.buf[8]=0x00;														//ÊÇ×îºóÒ»¸öÃüÁî
				  TxDat.buf[9]=Ex_Table.stu[chl];								//ÊÇ×îºóÒ»¸öÃüÁî
				
				  TxDat.buf[10]=Crc8CheckSum(TxDat.buf,10);			//CRC8Ð£Ñé
					TxDat.cnt=11;																	//·¢ËÍ×ÜµÄ×Ö½ÚÊý
					TxDat.f_tx=1;
			}
			else
			{
...
void IT1_Learn54Key(void)
{
				...
				//²»ÒªÇóÎÞÏß·¢ËÍ£¬Ö»µÈ´ýÒ£¿ØÆ÷·¢ËÍ
				Exchangebuf.type= EX_KEY_54KEY;                                    //Éè±¸ÀàÐÍ
				Exchangebuf.addr= 0;;
...
void IT1_DevPara_Report(unsigned char chl,unsigned char addr,unsigned char len)
{
	  ...
	  //uint8_t tt;
		...
		ps->buf[2]=Master_Address;							//Ä¿±êµØÖ·
		ps->buf[3]=0x00;												//Í¬²½×Ö
		ps->buf[4]=Dev.addr[0];									//Ô´µØÖ·
		ps->buf[5]=0x80+chl;												//Éè±¸Í¨µÀºÍ×ÜÊý
		...
		{
				ps->buf[6]=0x09;										 //Êý¾Ý³¤¶È
				ps->buf[7]=addr;										 //Êý¾ÝÆðÊ¼µØÖ·			
				ps->buf[8]=0x07;										 //²ÎÊý¸öÊý
...
//======================================================================================================
//°´ÐÂ·½Ê½×¢²áÊ±£¬È¡µÃÃ¿¸öÍ¨µÀµÄÎ»ÖÃºÍÀàÐÍ£¬²¢°´ÀàÐÍºÏ²¢
#ifdef USE_NEWREGISTER
void IT1_Register_Refresh(void)
{
	      ...
	      index=0;
				...
				while(index<8)
				{
						...
						else	//Èç¹ûÍ¨µÀ¿ÕÏÐ£¬¸ÄÎª0xb140 20170628
						{
				...
				if((Dev.type&0xf000)==0xb000)
				{
						if((Ex_Table.Exchange[index].type==EX_KEY_54KEY)||(Ex_Table.Exchange[index].type==EX_KEY_42KEY))
						{
							  if(Ex_Table.Exchange[index].type==EX_KEY_54KEY)num=7;
								...
								ps->buf[0] =0x27;											//Ö¡¿ØÖÆ,´ÓÉè±¸ÇëÇóÃüÁî
								ps->buf[1] =0x00;
								ps->buf[2] =0x00;
								ps->buf[3] =Dev.serial++;							//Ö¡ÐòÁÐºÅ
								ps->buf[4] =Dev.addr[0];							//ÍøÂçID
								ps->buf[5] =0x80+index+1;											//Éè±¸×ÜÍ¨µÀºÍµ±Ç°Í¨µÀ
								ps->buf[6] =3+num;						      	//Êý¾Ý³¤¶È
								ps->buf[7]=0;													//LED×´Ì¬						
								ps->buf[8] =0;							  				//µ±Ç°ÁÁ¶È  												
							  for(i=0;i<num;i++)ps->buf[9+i]=Pannel.item[index].onoffbuf[i];
								ps->buf[9+num]=Crc8CheckSum(ps->buf,9+num);		//CRC8Ð£Ñé
								ps->cnt=10+num;					                 	//·¢ËÍ×ÜµÄ×Ö½ÚÊý
						}
...
//==============================================================================================
//ÑÓÊ±·¢ËÍº¯Êý.Ã¿10mSµ÷ÓÃÒ»´Î 
void IT1_Dlytime_Handle(void)
{
		 uint8_t i;
	 
	   for(i=0;i<CHL_NUM;i++)						//Í¨µÀ0-´«¸ÐÆ÷´¥·¢£¬Í¨µÀ1-´«¸ÐÆ÷Í£Ö¹
		 {
			    //================================================================================================
					//ÑÓÊ±´¦Àí
